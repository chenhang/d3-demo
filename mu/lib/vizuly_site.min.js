...
function loadData() {
    d3.csv("data/mu.csv", function (a) {
        ...
        //            if (c.type = Number(c.type), c.value = Number(c.value), d < c.type) {
        //                for (var e = d; e < c.type; e++) {
        //                    b.push({
        //                        name: c.name,
        //                        value: 0,
        //                        type: String(e)
        ...
        //                }
        //            }
        //            d++, c.key = a.key, c.type = String(c.type), b.push(c)
        //        })
        //    }
        //    for (var d = b.length - 1; 32 > d; d++) b.push({
        //        key: a.key,
        //        value: 0,
        //        type: String(d)
        //    });
...
function initialize() {
    viz = vizuly.viz.corona(document.getElementById("viz_container")), viz.data(data).margin({
        top: "15%",
        left: "0%",
        right: "0%",
        bottom: "10%"
    }).outerRadius(350).y(function (a, b) {
        return a.value
    }).x(function (a, b) {
        return a.type
    }).interpolate("cardinal-closed").on("validate", onValidate).on("mouseover", onMouseOver).on("mouseout", onMouseOut).on("measure", onMeasure), theme = vizuly.theme.radial_linearea(viz).skin(vizuly.skin.LINEAREA_OCEAN), viz_title = viz.selection().select("svg").append("text").attr("class", "title").attr("y", 35).attr("text-anchor", "middle").text("Manchester United"), viz_heading = viz.selection().select("svg").append("text").attr("class", "heading").attr("y", 55).attr("text-anchor", "middle").text("PL Player Statistics"), changeSize(d3.select("#currentDisplay").attr("item_value"))
}

function onValidate() {
    viz.xAxis().tickFormat(function (a) {
        return convertIndex(a)
    }), viz.yAxis().tickFormat(function (a) {
        return a + "%"
    }), viz.xAxis().ticks(33), viz.yAxis().ticks(6)
}

function onMeasure() {
    viz_title.attr("x", viz.width() / 2).style("font-size", function () {
        return viz.height() / 60
    }).style("fill", function () {
        return theme.skin().labelColor
    }), viz_heading.attr("x", viz.width() / 2).style("font-size", function () {
...
function onMouseOver(a, b) {
    var c = Math.round(viz.outerRadius() / 20),
        d = viz.selection().selectAll("defs"),
        e = viz.selection().selectAll(".vz-plot");
    d.selectAll(".vz-tip-path").remove(), e.selectAll(".my-tip").remove(), d.append("path").attr("class", "vz-tip-path").attr("id", function (a, b) {
        return viz.id() + "_tip_path_arc_1"
    }).attr("d", function () {
        return vizuly.svg.text.arcPath(1.05 * viz.radiusScale()(a.y + a.y0), viz.thetaScale()(viz.x()(a)))
    }), e.append("text").attr("class", "my-tip").style("font-size", .95 * c + "px").style("text-transform", "uppercase").style("font-family", "Open Sans").style("fill-opacity", .75).style("fill", function () {
        return theme.skin().labelColor
    }).append("textPath").attr("startOffset", "50%").style("overflow", "visible").attr("xlink:href", function (a, b) {
        return "#" + viz.id() + "_tip_path_arc_1"
    }).text(function () {
        return viz.xAxis().tickFormat()(viz.x()(a))
    }), e.append("text").attr("class", "my-tip").attr("y", 1.5 * c).style("font-size", c + "px").style("fill", function () {
        return theme.skin().labelColor
    }).text(function () {
        return "Better than: " + viz.yAxis().tickFormat()(viz.y()(a)) + " teammates"
    }), e.append("text").attr("class", "my-tip").attr("y", 1.5 * -c).style("font-size", c + "px").style("fill-opacity", .75).style("fill", function () {
        return theme.skin().labelColor
    }).text(function () {
        return a.name
    }), e.append("text").attr("class", "my-tip").style("font-size", c + "px").style("fill-opacity", .75).style("fill", function () {
        return theme.skin().labelColor
    }).text(function () {
        return a.type + ': ' + a.actual_value
    })
}

function onMouseOut(a, b) {
    viz.selection().selectAll(".vz-tip-path").remove(), viz.selection().selectAll(".my-tip").remove()
}

function convertIndex(a) {
    return a
}

function changeSkin(a) {
    a && (theme.skin(a), viz_title.style("fill", theme.skin().labelColor), viz_heading.style("fill", theme.skin().labelColor), theme.apply())
}
...
function changeShape(a) {
    viz.interpolate(a).update()
}

function changeSize(a) {
    var b = String(a).split(",");
    viz_container.transition().style("width", b[0] + "px").style("height", b[1] + "px");
    var c = .75 * Math.min(Number(b[0]) / 2, Number(b[1]) / 2);
    viz.width(b[0]).height(b[1]).outerRadius(c).innerRadius(.4 * c).update()
}
var vizuly = {};
vizuly.version = "1.0", vizuly.ui = {}, vizuly.viz = {}, vizuly.viz.layout = {}, vizuly.viz.layout.CLUSTERED = "CLUSTERED", vizuly.viz.layout.STACKED = "STACKED", vizuly.viz.layout.OVERLAP = "OVERLAP", vizuly.viz.layout.STREAM = "STREAM", vizuly.color = {}, vizuly.color.shift = function (a, b) {
    return a = "0x" + a.replace("#", ""), a = parseInt(a, 16), a += 65793, a |= b, "#" + a.toString(16)
}, vizuly.component = {}, vizuly.component.create = function (a, b, c, d) {
    b.parent = a, b.properties = c, b.id = vizuly.util.guid(), b.selection = d3.select(a).append("div").attr("id", "div_" + b.id).style("width", "100%").style("height", "100%");
    var e = [];
    e.push("mouseover"), e.push("mouseout"), e.push("mousedown"), e.push("click"), e.push("dblclick"), e.push("touch"), e.push("zoom"), e.push("zoomstart"), e.push("zoomend"), e.push("initialize"), e.push("validate"), e.push("measure"), e.push("update"), Object.getOwnPropertyNames(c).forEach(function (a, b, c) {
        e.push(a + "_change")
    ...
    return setProps = function (a, b, c) {
        ...
        if (!a) {
            ...
            if (Object.getOwnPropertyNames(c).forEach(function (c) {
                ...
                }), a.length > 0) throw new Error("vizuly.util.component.validate(): " + a.concat() + " need to be declared");
            b.dispatch.validate()
        }
    }, b.dispatch.component = f(), b.dispatch.component
}, vizuly.data = {}, vizuly.data.aggregateNest = function (a, b, c) {
    function d(a, b) {
        if (b)
    ...
    return size = {}, size.width = b - vizuly.util.measure(a.left, b) - vizuly.util.measure(a.right, b), size.height = c - vizuly.util.measure(a.top, c) - vizuly.util.measure(a.bottom, c), size.top = vizuly.util.measure(a.top, c), size.left = vizuly.util.measure(a.left, b), size.bottom = vizuly.util.measure(a.bottom, c), size.right = vizuly.util.measure(a.right, b), size
...
}, vizuly.util.createCSSKey = function (a) {
    ...
    var e = d3.svg.arc().innerRadius(a).outerRadius(a)(d),
        ...
        g = f.exec(e);
    return g && (g = g[0]), g
}, vizuly.svg.text.wrap = function (a, b, c) {
    c = !c && a.attr("width") ? Number(a.attr("width")) : c, a.each(function () {
        var a = d3.select(this);
        a.selectAll("tspan").remove();
        var d, e = a.text().split(/\s+/).reverse(),
            f = [],
            g = 0,
            h = a.attr("x"),
            i = a.attr("y"),
            j = parseFloat(b);
        isNaN(j) && (j = 0);
        for (var k = a.text(null).append("tspan").attr("x", h).attr("y", i).attr("dy", "0px"); d = e.pop();) f.push(d), k.text(f.join(" ")), k.node().getComputedTextLength() > c && (g++, f.pop(), k.text(f.join(" ")), f = [d], k = a.append("tspan").attr("class", "vz-multi-line").attr("x", h).attr("y", i).attr("dy", +j + "px").text(d))
    })
}, vizuly.theme = {}, vizuly.skin = {}, vizuly.skin.COLUMNBAR_AXIIS = "Axiis", vizuly.skin.COLUMNBAR_NEON = "Neon", vizuly.skin.COLUMNBAR_MATERIALBLUE = "MaterialBlue", vizuly.skin.COLUMNBAR_MATERIALPINK = "MaterialPink", vizuly.theme.column_bar = function (a) {
    function b() {
    ...
    function c() {
        var b = a.width(),
            c = a.selection();
        m = Math.max(8, Math.round(a.width() / 65)), c.attr("class", q["class"]), c.selectAll(l + " .tick text").style("font-weight", q.ordinalAxis_font_weight).style("fill", q.labelColor).style("fill-opacity", 1).style("font-size", m + "px").style("opacity", function () {
            return b > 399 ? 1 : 0
        }), c.selectAll(k + " line").style("stroke", q.valueAxis_line_stroke).style("stroke-width", 1).style("opacity", q.valueAxis_line_opacity), c.selectAll(k + " text").style("font-size", m + "px").style("fill", q.labelColor).style("fill-opacity", .8);
    ...
    var k, l, m, n = {
            ...
            Axiis: {
                ...
                labelColor: "#444",
                color: "#000",
                colorScale: d3.scale.linear().range(["#DF1133", "#3333DF"]).domain([0, a.data()[0].length]),
                ...
                bar_fill: function (b, c) {
                    var d = vizuly.svg.gradient.fade(a, vizuly.color.shift(this.colorScale(a.xScale().domain().indexOf(a.x().apply(this, [b]))), 2245632 * c));
                    return "url(#" + d.attr("id") + ")"
                },
                bar_fill_opacity: function (a, b) {
                    return 1 - b / 4
                },
    ...
    b();
    var p = vizuly.svg.gradient.blend(a, "#000", "#000");
    b.apply = function (a) {
    ...
    return b.skin = function (a) {
        ...
        if (!n[a]) throw new Error("theme/column_bar.js - skin " + a + " does not exist.");
        return q = n[a], b
    }, b.skins = function () {
        return n
    }, b
    ...
    return b
}, vizuly.skin.WEIGHTED_TREE_AXIIS = "Axiis", vizuly.viz.corona = function (a) {
    function b() {
    ...
    function c() {
        h.validate(), i = vizuly.util.size(f.margin, f.width, f.height), k = Math.round(f.data[0].length / f.xAxis.ticks()[0]), l = function () {
            for (var a = [], b = 0; b < f.xAxis.ticks()[0]; b++) a.push(b);
            return a
        }.apply(this);
        var a = f.layout == vizuly.viz.layout.STACKED ? "reverse" : "none";
        if (m = d3.layout.stack().values(function (a) {
                return a
            }).x(function (a) {
                return String(f.x(a))
            }).y(function (a) {
            ...
            var b;
            "string" == typeof h.x()(f.data[0][0]) ? (b = d3.scale.ordinal(), b.rangeBands([0, 2 * Math.PI])) : h.x()(f.data[0][0]) instanceof Date ? (b = d3.time.scale(), b.range([0, 2 * Math.PI])) : (b = d3.scale.linear(), b.range([0, 2 * Math.PI])), f.thetaScale = b
        }
    ...
    function d() {
        c(), n.attr("width", f.width).attr("height", f.height), s.attr("width", f.width).attr("height", f.height), r.style("width", i.width).style("height", i.height).attr("transform", "translate(" + (i.left + i.width / 2) + "," + (i.top + i.height / 2) + ")"), w.style("width", i.width).style("height", i.height).attr("transform", "translate(" + i.left + "," + i.top + ")"), p.attr("transform", "translate(" + (i.left + i.width / 2) + "," + (i.height / 2 + i.top + 3) + ")"), q.attr("transform", "translate(" + (i.left + i.width / 2) + "," + (i.height / 2 + i.top + 3) + ")"), t.attr("width", i.width).attr("height", i.height);
        var a = u.selectAll(".vz-series-plots").data(f.data);
        a.enter().append("g").attr("class", "vz-series-plots"), a.exit().remove(), a.each(function (a, b) {
            var c = d3.select(this),
                d = c.selectAll(".vz-line").data([a]);
            d.enter().append("path").attr("class", "vz-line"), d.exit().remove(), d.transition().duration(f.duration).attr("d", function (a, b) {
                return x(a)
            });
            var e = c.selectAll(".vz-area").data([a]);
            e.enter().append("path").attr("class", "vz-area"), e.exit().remove(), e.transition().duration(f.duration).attr("d", function (a, b) {
                return y(a)
            })
            ...
            c.each(function () {
                var a = d3.select(this);
                a.append("circle").attr("class", "vz-hit-circle").style("fill", "#FFF").style("stroke", null).style("opacity", .001).transition().attr("r", j)
            })
        }),
        ...
        p.selectAll(".vz-radial-x-axis-tick").remove(),
            p.selectAll(".vz-radial-x-axis-tick").data(l).enter().append("g").attr("class", "vz-radial-x-axis-tick").append("text").append("textPath").attr("text-anchor", "middle").attr("startOffset", "50%").style("overflow", "visible").attr("xlink:href", function (a, b) {
                return "#" + f.id + "_x_text_arc_" + b
            }).text(function (a, b) {
                return f.xAxis.tickFormat()(f.x(f.data[0][b * k]))
            }),
            q.selectAll(".vz-label-ticks").remove(), q.append("g").attr("class", "vz-label-ticks").style("display", "none").call(f.yAxis), q.selectAll(".vz-y-axis-tick").remove();
        console.log(l);
        var b = q.selectAll(".tick")[0].map(function (a) {
    ...
    function e(a, b) {
        var c = f.radiusScale(a),
            d = f.thetaScale(b) - Math.PI / 2;
        return a = c * Math.cos(d) + i.width / 2, b = c * Math.sin(d) + i.height / 2, {
            x: a,
            y: b
        }
    }
    ...
    var f = {},
        g = {
            data: null,
            layout: vizuly.viz.layout.OVERLAP,
            margin: {
                top: "7%",
                bottom: "7%",
                left: "8%",
        ...
        h = vizuly.component.create(a, f, g);
    h.type = "viz.chart.corona";
    var i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x = d3.svg.line.radial(),
        y = d3.svg.area.radial();
    return b(), h.update = function () {
        return d(), h
    }, h
};
var viz, data, viz_container, viz_title, viz_heading, theme;